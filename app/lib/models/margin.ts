...
export async function getmarginAktif(): Promise<ApiResponse<Margin[]>> {
  ...
  try {
    ...
    return {
      status: 500,
      error: `Failed to fetch active margins: ${
        error instanceof Error ? error.message : "Unknown error"
      }`,
    };
  } finally {
...
export async function createMargin(
  data: Omit<Margin, "idmargin_penjualan">
  ...
  try {
    ...
    await db.execute(
      "INSERT INTO margin_penjualan (iduser, persen, status, updated_at, created_at) VALUES (?, ?, ? , now(), now())",
      [iduser, persen, status]
    );
    return { status: 201, data: { message: "Margin Penjualan created" } };
  } catch (error) {
    return {
      status: 500,
      error: `Failed to create Margin: ${
        error instanceof Error ? error.message : "Unknown error"
      }`,
    };
  } finally {
...
export async function updateMargin(
  ...
  const { idmargin_penjualan, iduser, persen, status, updated_at } =
    parsed.data;
  const db = await getDbConnection();
  try {
    ...
    const [result] = await db.execute(
      "UPDATE margin_penjualan SET iduser = ?, persen = ?, status = ?, updated_at = now() WHERE idmargin_penjualan = ?",
      [iduser, persen, status, idmargin_penjualan]
    );
...
export async function deleteMargin(
  ...
  const db = await getDbConnection();
  try {
    const [penjualans] = await db.execute(
      "SELECT COUNT(*) as count FROM penjualan WHERE idmargin_penjualan = ?",
      [id]
    );
    if ((penjualans as any)[0].count > 0) {
      return {
        status: 400,
        error:
          "Tidak dapat menghapus margin yang masih terkait dengan penjualan",
      };
    }
    const [result] = await db.execute(
