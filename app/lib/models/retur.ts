...
import { returSchema } from "@/app/lib/utils/validation";
...
export async function getRetur(): Promise<ApiResponse<Retur[]>> {
  ...
  try {
  const [returs] = await db.execute(
    'select * from view_retur');

    return {
      status: 200,
      data: returs as Retur[],
    };
  } catch (error) {
    return { status: 500, error: `Failed to fetch Retur: ${error instanceof Error ? error.message : 'Unknown error'}` };
  } finally {
    db.release();
  }
}
