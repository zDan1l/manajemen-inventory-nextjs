...
import { penjualanSchema } from "@/app/lib/utils/validation";
...
export async function getPenjualan(): Promise<ApiResponse<Penjualan[]>> {
  ...
  const [penjualans] = await db.execute(
    ...
    return { status: 500, error: `Failed to fetch Pengadaan: ${error instanceof Error ? error.message : 'Unknown error'}` };
  } finally {
    db.release();
  }
}
