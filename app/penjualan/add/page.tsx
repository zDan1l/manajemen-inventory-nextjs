...
import { ApiResponse, BarangTersedia, Margin, DetailPenjualanInput } from '@/app/lib/type';
...
export default function AddPenjualanPage() {
  ...
  const handleBarangChange = (index: number, idbarang: number) => {
    ...
    const margin = marginList.find(m => m.idmargin_penjualan === selectedMargin);
  ...
  const addDetailRow = () => {
    setDetails([...details, { idbarang: 0, jumlah: 0, harga_jual: 0, sub_total: 0 }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasDuplicate) {
      warning('Terdapat barang yang sama di beberapa baris. Setiap barang hanya boleh dipilih sekali.');
      return;
    }
    ...
    for (const detail of validDetails) {
      ...
      if (detail.jumlah > barang.stok_tersedia) {
        warning(`Stok ${barang.nama_barang} tidak mencukupi. Tersedia: ${barang.stok_tersedia}`);
        return;
      }
      if (detail.jumlah <= 0) {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      const calculatedSubtotal = validDetails.reduce((sum, d) => sum + d.sub_total, 0);
        ...
        <Card>
          <div className="p-6 text-center">
            <div className="flex justify-center items-center gap-2">
              ...
              <span className="text-gray-600">Memuat data barang dan margin...</span>
            </div>
          </div>
        </Card>
      )}
        ...
        <Card>
          <div className="p-6 bg-yellow-50 border-l-4 border-yellow-400">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-yellow-800">Tidak ada barang tersedia</p>
                <p className="text-sm text-yellow-700 mt-1">Silakan lakukan penerimaan barang terlebih dahulu untuk dapat membuat penjualan.</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="p-6 bg-yellow-50 border-l-4 border-yellow-400">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-yellow-800">Tidak ada margin aktif</p>
                <p className="text-sm text-yellow-700 mt-1">Silakan tambahkan margin penjualan aktif terlebih dahulu.</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="bg-gradient-to-r from-[#00A69F] to-[#0D9488] text-white p-6 rounded-t-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm mt-1">Buat transaksi penjualan dengan pilihan barang yang tersedia</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Masukkan persentase PPN (0-100%). Nilai PPN akan dihitung otomatis dari subtotal.
                  </p>
                </div>
              </div>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                      <th className="p-3 text-left text-sm font-semibold text-gray-700">Barang & Satuan</th>
                      <th className="p-3 text-center text-sm font-semibold text-gray-700 w-24">Stok</th>
                      <th className="p-3 text-right text-sm font-semibold text-gray-700 w-32">Harga Jual</th>
                      <th className="p-3 text-center text-sm font-semibold text-gray-700 w-24">Jumlah</th>
                      <th className="p-3 text-right text-sm font-semibold text-gray-700 w-32">Sub Total</th>
                      <th className="p-3 text-center text-sm font-semibold text-gray-700 w-24">Aksi</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      >
                        ...
                        <td className="p-3 text-right">
                          <span className="text-sm text-gray-700">
                            {detail.idbarang > 0 ? formatCurrency(detail.harga_jual) : '-'}
                          </span>
                        </td>
                        <td className="p-3 text-center">
                          <input
                        ...
                        <td className="p-3 text-right">
                          <span className="text-sm font-semibold text-gray-700">
                            {detail.sub_total > 0 ? formatCurrency(detail.sub_total) : '-'}
                          </span>
                        </td>
                        <td className="p-3 text-center">
                          {details.length > 1 && (
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg">
              <h3 className="text-md font-semibold text-gray-800 mb-3">Ringkasan Transaksi</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                <div className="bg-white p-3 rounded-lg shadow-sm border border-blue-200">
                  ...
                  <p className="text-lg font-bold text-[#00A69F]">{totalItems}</p>
                </div>
                <div className="bg-white p-3 rounded-lg shadow-sm border border-blue-200">
                ...
                <div className="bg-white p-3 rounded-lg shadow-sm border border-blue-200">
                  ...
                  <p className="text-lg font-bold text-gray-700">{formatCurrency(subtotalNilai)}</p>
                </div>
                <div className="bg-white p-3 rounded-lg shadow-sm border border-blue-200">
                  <p className="text-xs text-gray-600 mb-1">PPN ({ppn}%)</p>
                  <p className="text-lg font-bold text-gray-700">{formatCurrency(ppnNilai)}</p>
                </div>
              </div>
              <div className="bg-white p-3 rounded-lg shadow-md border-2 border-[#00A69F]">
                <div className="flex justify-between items-center">
                  <p className="text-sm font-semibold text-gray-700">Total Bayar</p>
                  <p className="text-2xl font-bold text-[#00A69F]">{formatCurrency(totalNilai)}</p>
                </div>
              </div>
              {totalItems > 0 && (
                <p className="text-xs text-gray-600 mt-3 flex items-center gap-2">
                  ...
                  Preview perhitungan harga. Nilai final akan dihitung saat menyimpan.
                </p>
              )}
            </div>
