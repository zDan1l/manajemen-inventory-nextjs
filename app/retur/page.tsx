    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#00A69F] to-[#0D9488] rounded-2xl shadow-lg p-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-teal-100 mt-1">Kelola pengembalian barang yang rusak atau tidak sesuai</p>
          </div>
        </div>
      </div>
      ...
      <Table
        data={returs.map(retur => ({
          ...
          total_nilai_retur: retur.total_nilai_retur ? formatCurrency(retur.total_nilai_retur) : 'Rp 0',
        }))}
        columns={columns}
        idKey="idretur"
        isTransaction={true}
      ...
      {returs.length === 0 && !loading && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-2">Klik tombol "Tambah Retur Baru" untuk memulai</p>
          </div>
        </div>
      )}
    </div>
